...
# ✅ ADDED: Stop flag for graceful shutdown
...
# ✅ ADDED: Mute telebot spam logs ("Break infinity polling")
logging.getLogger("telebot").setLevel(logging.CRITICAL)
logging.getLogger("urllib3").setLevel(logging.WARNING)

BOT_TOKEN = "8632922565:AAF659EYJNbCAFlzYpu6bXJf8Uqrn5x0Z7U"
...
# ✅ ADDED: graceful shutdown handler (Ctrl+C / SIGTERM)
def _graceful_shutdown(signum=None, frame=None):
    try:
        ...
        try:
            ...
            pass
        logger.info("🛑 Shutdown signal received. Bot stopped cleanly.")
    finally:
        try:
            conn.close()
        except Exception:
...
def contact_handler(message):
    bot.send_message(message.chat.id, "📞 Contact: @Nikunjbro")
...
def start_bot():
    ...
    while True:
        ...
        # ✅ ADDED: Ctrl+C -> clean stop (NO SPAM, NO RESTART LOOP)
        except KeyboardInterrupt:
            logger.info("🛑 Bot stopped by user (Ctrl+C).")
            STOP_EVENT.set()
            break

        # ✅ ADDED: SIGTERM/SystemExit -> clean stop
        except SystemExit:
            STOP_EVENT.set()
            break
        ...
        # ✅ ADDED: if stopped, exit loop
        if STOP_EVENT.is_set():
            break

# Main execution
if __name__ == "__main__":
    logger.info("Starting 24x7 THB Hosting Bot...")
    start_bot()